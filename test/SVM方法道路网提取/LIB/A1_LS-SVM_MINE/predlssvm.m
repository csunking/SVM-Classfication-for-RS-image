...
if model.preprocess(1)=='p'
    x = postlssvm(model,model.xtrain);
else
    x = model.xtrain;
end
...
Yphat = simlssvm(model,Xt);

% output of the smoother on training data Yhat
Yhat = simlssvm(model,x);

biascorr = S*Yhat-Yphat;

% bias corrected smoother ybc
ybc = Yphat - biascorr;
...
if conftype(1)=='s'
    if model.preprocess(1)=='p'
        model.xtrain = prelssvm(model,Xt); model.nb_data=size(Xt,1);
    end
    z = tbform(model,alpha);
elseif conftype(1)=='p'
    z = norminv(alpha/2);
else
